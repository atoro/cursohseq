...
<div class="contenedor">
	...
	<div class="contenedor_info">
		<div class="inicio1">
        <?php
          $listados = "select * from  subcategoria where id_subcategoria ='$_GET[subcat]'";
          $sentencias = mysql_query($listados,$conn);
          if($rss=mysql_fetch_array($sentencias,$mibase)){
        ?>
			<h3><img src="imagenes/tic.png"> <?php echo $rss["subcategoria"]; ?> </h3>
      		<?php
		  }
        ?>
        
        <?php
          $listados = "select * from  servicio where categoria ='$_GET[cat]' and subcategoria ='$_GET[subcat]'";
          $sentencias = mysql_query($listados,$conn);
          while($rss=mysql_fetch_array($sentencias,$mibase)){
        ?>
      <div class="imgcliente">
        <img src="imagenes/cursos/<?php echo $rss["id"]; ?>.jpg">
        <h2><?php echo $rss["nombre_servicio"]; ?></h2>
        <a style="margin: 0 auto 10px auto;" href="detallecurso?id=<?php echo $rss["id"] ?>&cat=<?php echo $_GET["cat"] ?>&subcat=<?php echo $_GET["subcat"] ?>.php">Ver Detalle</a>
      </div>
       <?php } ?>
